...
test:
	...
	R --slave -e 'Sys.setenv(NOT_CRAN="true"); library(testthat); setwd(file.path(.libPaths()[1], "knudson", "tests")); system.time(test_check("knudson", filter="${file}", reporter=ifelse(nchar("${r}"), "${r}", "summary")))'
...
check: build
	-R CMD check --as-cran knudson_$(VERSION).tar.gz
	rm -rf knudson.Rcheck/
